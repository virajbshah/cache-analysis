...
LoopCacheAnalysis::getLoopCacheCost(affine::AffineForOp *LoopOp) {
  ...
  for (auto &Op : InnerMostLoop.getOps()) {
    ...
    if (auto LoadOp = llvm::dyn_cast<affine::AffineLoadOp>(Op)) {
      ...
      AffineMap MappedIndices = LoadOp.getAffineMap().shiftSymbols(2);
      const auto &[Strides, Offset] =
          LoadOp.getMemRefType().getStridesAndOffset();

      llvm::SmallVector<unsigned> Levels;
      for (Value Index :
           LoadOp.getIndices().take_front(MappedIndices.getNumDims())) {
      ...
      AffineExpr LinearizedIndex =
          getAffineConstantExpr(0, LoadOp->getContext());
      ...
      void *BasePointer = LoadOp.getMemref().getAsOpaquePointer();
      // Assuming the base pointer is aligned.
      AffineExpr CacheLineExpr = LinearizedIndex.floorDiv(
          getAffineSymbolExpr(CacheLineSizeSymbolIndex, LoadOp->getContext()));
      ...
      auto TotalDistance =
          ...
          (LoadOp.getMemRefType().getElementTypeBitWidth() / 8);
